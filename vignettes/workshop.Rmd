---
title: "Streamline spatial and transcriptional analysis with tidyomics"
...
## Base R and the `data.frame`

The base R `data.frame` we are all familiar with.
...
tibble provides a modernised `data.frame` with a more useful appearance. It also reduces the unexpected behviour sometimes produced by the `data.frame`, such as the silent conversion to a vector when subsetting a single colum.
