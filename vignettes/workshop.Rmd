...
spe_obj <-
  spe_obj |>
  gate(size = 0.2, alpha = 0.8, rasterise_points = TRUE)
```
...
spe_obj_aggregated_de |>
    ...
    mutate(significant_de = ifelse(`PValue___area2` < 0.05 & (`logFC___area2` > 1 | `logFC___area2` < -1), TRUE, FALSE)) |> 
    
    # Label each feature with its name if it is beyond a given threshold
    mutate(label = ifelse(`PValue___area2` < 0.05 & (`logFC___area2` > 3 | `logFC___area2` < -3), feature, NA)) |> 

    # Create the volcano plot
    ggplot(aes(x = `logFC___area2`, y = `PValue___area2`, label = label, colour = significant_de)) +
    geom_point() +
    geom_text_repel(colour = "blue") +
    scale_y_continuous(trans = compose_trans("log10", "reverse")) +
    scale_colour_manual(values = c("grey", "black")) +
    labs(title = str_c("DE contrast area 2"), x = "Log2 fold change", y = "P-Value") +
    theme_bw() +
    theme(legend.position = "bottom")
```
